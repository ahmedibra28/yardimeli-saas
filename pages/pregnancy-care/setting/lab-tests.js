...
const LabTests = () => {
  ...
  const { getLabTests, postLabTest, updateLabTest, deleteLabTest } =
    useLabTestsHook({
      page,
      q,
    })
  ...
  const { data, isLoading, isError, error, refetch } = getLabTests
  ...
  const editHandler = (labTest) => {
    setId(labTest._id)
    setEdit(true)
    setValue('name', labTest.name)
    setValue('reference', labTest.reference)
    setValue('isActive', labTest.isActive)
  }
  ...
  return (
    <>
      ...
      {isSuccessPost && (
        <Message variant='success'>
          Lab test has been Created successfully.
        </Message>
      )}
